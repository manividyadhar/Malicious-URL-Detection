 ...
 */
function validateURL(url: string): { valid: boolean; parsed: URL | null; error?: string } {
 ...
 */
function hasURLShortener(hostname: string): boolean {
    const hostnameLower = hostname.toLowerCase();
 ...
 */
function extractFeatures(url: string, parsed: URL): URLFeatures {
 ...
 */
function calculateRiskScore(features: URLFeatures): { score: number; reasons: string[] } {
    let score = 0;
 ...
 * ```
 */
export function analyzeURL(url: string, _includeFeatures?: boolean): AnalysisResult {
    // Validate URL
 ...
 */
export function analyzeURLBatch(urls: string[], _includeFeatures?: boolean): AnalysisResult[] {
